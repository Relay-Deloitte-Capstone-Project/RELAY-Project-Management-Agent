...
-- prisma/schema.prisma), not in this Postgres instance, so project_staffing
...
CREATE TABLE IF NOT EXISTS public.project_staffing (
...
-- check).
CREATE INDEX IF NOT EXISTS idx_project_staffing_engagement
    ON public.project_staffing (engagement_id);
CREATE INDEX IF NOT EXISTS idx_project_staffing_email
    ON public.project_staffing (email);
